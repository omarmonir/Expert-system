...
namespace FacultyManagementSystemAPI.Controllers
{
	...
	public class CourseController(ICourseService courseService) : ControllerBase
	{
		...
		[HttpGet("GetCoursesByFacultyId/{facultyId}")]
		public async Task<IActionResult> GetCoursesByProfessorIdWithPreCourseName(int facultyId)
		{
			if (!ModelState.IsValid)
			{
				return BadRequest(ModelState);
			...
			try
			{
				var courses = await _courseService.GetCoursesByProfessorIdWithPreCourseNameAsync(facultyId);
				return Ok(courses);
			}
			catch (Exception ex)
			{
